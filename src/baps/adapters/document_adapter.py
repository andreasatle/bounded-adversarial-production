...
def build_module_research_tools(
    filters: list[str],
    list_desc: str,
    fetch_desc: str,
    entity_desc: str,
    ...
    section_summaries = [
        {
            ...
            "body": sanitize_model_string(section.body),
        }
...
class DocumentProjectAdapter:
    ...
    def create_initial_state(self, config: dict[str, object]) -> State:
        ...
        return State(
            artifacts=(DocumentArtifact(id=config_artifact_id(config), sections=()),),
        )
        ...
        if tool_name == "list_modules":
            ...
            for sec in artifact.sections:
                ...
                if filter_val == "summary":
                    sentence = first_sentence(sec.body)
                    entry += f"\n  {sentence}"
                lines.append(entry)
            return "\n".join(lines)
