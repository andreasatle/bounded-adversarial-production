...
def _render_verification_block(
    ...
    if result is None:
        return ""
    verification_json = json.dumps(verification_result_to_dict(result), sort_keys=True)
    return f"- verification_result_json: {verification_json}\n{guidance}"


def render_create_game_prompt(
    config: RunConfig,
    state: State,
    ...
    return (
        ...
        "\n"
        f"- artifact_id: {config_artifact_id(config)}\n\n"
...
def render_create_game_red_prompt(
    state_view: StateView,
    game_spec: GameSpec,
    config: RunConfig,
) -> str:
...
def render_red_prompt_supplement_with_adapter(
...
def render_referee_prompt_supplement_with_adapter(
    adapter: ProjectTypeAdapter,
    state_view: StateView,
    game_spec: GameSpec,
    delta_state: DeltaState,
...
def render_red_prompt(
    ...
    state_view_json = json.dumps(
        {
            "id": state_view.id,
            "content": state_view.content,
            "input_fingerprint": state_view.input_fingerprint,
        },
        sort_keys=True,
    )
...
def render_create_game_research_prompt(state_view: StateView, config: RunConfig) -> str:
