...
class IterationRunResult(BaseModel):
    """Result returned by run_project_iterations, summarising outcome fields across all iterations."""
...
def _solve_gap(
    ...
    try:
        ...
        debug_print_northstar_update_proposal(exc.rationale, exc.proposed_northstar)
        append_northstar_proposal_to_blackboard(
            workspace=config.workspace,
    ...
    if isinstance(result, DecomposeSpec):
        if depth >= max_depth:
            logger.info(
                "[solve_gap] max_depth=%d reached, cannot decompose further; stopping.",
                max_depth,
            )
    ...
    ctx.verification_result = verify_export_with_adapter(
        adapter, output_path, updated_state, artifact_id, sandbox_mode=sandbox_mode
    )
    debug_print_verification_result(ctx.verification_result)
    if ctx.output_changed:
        commit_export_with_adapter(adapter, output_path, game_spec)
