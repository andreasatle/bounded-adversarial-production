...
def test_audit_play_game_state_view_missing_source_path_graceful(
    tmp_path: Path,
) -> None:
    adapter = AuditProjectAdapter()
    state = adapter.create_initial_state(
...
def test_audit_referee_prompt_supplement_has_accept_reject_criteria(
    tmp_path: Path,
) -> None:
    adapter = AuditProjectAdapter()
    state = adapter.create_initial_state(
...
def test_render_source_content_unknown_extension_uses_empty_fence(
    tmp_path: Path,
) -> None:
    f = tmp_path / "Makefile"
    f.write_text("all:\n\techo done\n")
