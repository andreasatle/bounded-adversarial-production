...
from baps.core.clients import SpecRole, _parse_spec_roles
...
def test_summarize_role_name_accepted_by_parse_spec_roles() -> None:
    """SpecRole.SUMMARIZE is in the valid roles set; _parse_spec_roles must not raise."""
    result = _parse_spec_roles({
...
def test_resolve_summarize_role_returns_none_when_no_role_configured(tmp_path: Path) -> None:
...
def test_resolve_summarize_role_returns_role_when_configured(
    ...
    monkeypatch.setattr(
        "baps.core.runtime._build_client_for_role",
...
def test_summarization_context_is_passed_to_play_game(
    ...
    """_run_project_iterations must forward summarization_context to play_game."""
    ...
    _run_project_iterations(config, adapter, service, state, summarization_context=ctx)
...
def test_stop_reason_same_with_and_without_summarizer(
    ...
    def _run(workspace: Path, ctx: SummarizationContext | None) -> StopReason:
        ...
        service, state = _initialize_project(config)
        result = _run_project_iterations(
            config, DocumentProjectAdapter(), service, state,
