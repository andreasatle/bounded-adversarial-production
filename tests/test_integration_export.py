    ...
    state = state_module.State(
        ...
        artifacts=(
            state_module.CodingArtifact(
                ...
                files=(
                    state_module.CodeFile(
                        path="tests/test_fibonacci.py", content=escaped_pytest
                    ),
                ),
            ),
        ),
...
def test_coding_adapter_verify_export_discovers_and_runs_pytest_tests(
    ...
    result = adapter.verify_export(
        output_dir, state, "main-codebase", sandbox_mode="none"
    )
...
def test_coding_adapter_verify_export_runs_pytest_and_captures_success(
    ...
    def _fake_run(args, cwd, capture_output, text, check):
        ...
        return subprocess.CompletedProcess(
            args=args, returncode=0, stdout="2 passed\n", stderr=""
        )
...
def test_coding_adapter_verify_export_captures_failure(
    ...
    def _fake_run(args, cwd, capture_output, text, check):
        return subprocess.CompletedProcess(
            args=args, returncode=1, stdout="1 failed\n", stderr="traceback\n"
        )
    ...
    result = adapter.verify_export(
        tmp_path / "project", state, "main-codebase", sandbox_mode="none"
    )
    assert result is not None
    assert result.exit_code == 1
    assert result.passed is False
    ...
    state = state_module.State(
        ...
        artifacts=(
            state_module.CodingArtifact(
                ...
                files=(
                    state_module.CodeFile(
                        path="src/fibonacci.py", content="def fibonacci(n): return n\n"
                    ),
                ),
            ),
        ),
...
def test_coding_adapter_commit_export_inits_and_commits(
    monkeypatch, tmp_path: Path
) -> None:
    import baps.adapters.coding_adapter as coding_module
...
def testcommit_export_with_adapter_calls_adapter_method(
    monkeypatch, tmp_path: Path
) -> None:
...
def testcommit_export_with_adapter_skips_adapter_without_method(tmp_path: Path) -> None:
    ...
    result = commit_export_with_adapter(
